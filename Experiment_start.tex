...
\begin{numcases}
 {}
 \frac{N^2\beta}{\alpha+N^2\beta}\times N=\frac{N^3\beta}{\alpha+N^2\beta}(励起子なし)\\
 \frac{N\beta}{\alpha+N\beta}\times N=\frac{N^2\beta}{\alpha+N\beta}(励起子あり)
\end{numcases}

となる。したがって、発光強度は
\begin{numcases}
 {}
 \frac{N^3(\alpha+\beta)}{\alpha+N^2\beta}(励起子なし)\\
 \frac{N^2(\alpha+\beta)}{\alpha+N\beta}(励起子あり)
\end{numcases}
...
\begin{enumerate}%箇条書き環境下では文頭では自力で空白を入れる
 \item 300 g/mmの回折格子を用いた測定
       ...
       縦軸は入射光強度、横軸はスリット幅である。入射光強度には、発光スペクトルを積分した値を用いた。スリット幅が大きいほど強い光が入射していることが分かる。スリット幅が200 $\upmu$mより広い部分で、入射光強度の増加量が減っている。これは、光ファイバーのコアの幅が有限であるためである。スリット幅がファイバーのコアの直径より狭いうちは、スリット幅に比例して入射光強度が大きくなるが、コアの直径を超えた時点で、入射光強度はコアの直径によって定まってしまうため、スリット幅では入射光強度を調整できなくなる。\\
